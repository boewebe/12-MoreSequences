...
def run_test_pizza():
    ...
    # -------------------------------------------------------------------------
    title = 'PIZZA test 4:  135 slices, thin (thickness=1) black lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 200)
    circle.outline_thickness = 1
    circle.fill_color = 'yellow'
    pizza(window, circle, 135, 'black', 3)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    title = ('POLYGON test 3: 6 segments with thin yellow lines')
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(50, 50), 25)
    circle.outline_thickness = 2
    polygon(window, circle, 6, 'yellow', 1)
    window.continue_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    title = ('FANCY POLYGON test 4:  20 black lines on yellow circle,'
             + ' hops = 5.')
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'yellow'
    fancy_polygon(window, circle, 20, 5, 'black', 2)
    window.close_on_mouse_click()

    # Test 5:

    title = ('FANCY POLYGON test 5:  51 red lines on blue circle,'
             + ' hops = 25.')
    window = rg.RoseWindow(480, 350, title)
    ...
    title = ('FANCY POLYGON test 6:  300 purple lines on green circle,'
             + ' hops = 61.')
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 200)
    circle.fill_color = 'green'
    fancy_polygon(window, circle, 300, 61, 'purple', 2)
    window.close_on_mouse_click()
